...
func TestOpenFgaClientWriteClientWriteConflictOptions(t *testing.T) {
	...
	t.Run("Client Write with WritesOnDuplicate ignore option", func(t *testing.T) {
		test := TestDefinition{
			Name:           "Write",
			JsonResponse:   `{}`,
			ResponseStatus: 200,
		...
		options := ClientWriteOptions{
			Conflict: ClientWriteConflictOptions{
				WritesOnDuplicate: CLIENT_WRITE_REQUEST_WRITES_ON_DUPLICATE_IGNORE,
			},
		}
	...
	t.Run("Client Write with WritesOnDuplicate error option", func(t *testing.T) {
		test := TestDefinition{
			Name:           "Write",
			JsonResponse:   `{}`,
			ResponseStatus: 200,
		...
		options := ClientWriteOptions{
			Conflict: ClientWriteConflictOptions{
				WritesOnDuplicate: CLIENT_WRITE_REQUEST_WRITES_ON_DUPLICATE_ERROR,
			},
		}
		_, err := fgaClient.Write(context.Background()).Body(body).Options(options).Execute()
		if err != nil {
	...
	t.Run("Client Write with DeleteOnMissing ignore option", func(t *testing.T) {
		test := TestDefinition{
			Name:           "Write",
			JsonResponse:   `{}`,
			ResponseStatus: 200,
		...
		options := ClientWriteOptions{
			Conflict: ClientWriteConflictOptions{
				DeleteOnMissing: CLIENT_WRITE_REQUEST_DELETES_ON_MISSING_IGNORE,
			},
		}
		_, err := fgaClient.Write(context.Background()).Body(body).Options(options).Execute()
		if err != nil {
	...
	t.Run("Client Write with DeleteOnMissing error option", func(t *testing.T) {
		test := TestDefinition{
			Name:           "Write",
			JsonResponse:   `{}`,
			ResponseStatus: 200,
		...
		options := ClientWriteOptions{
			Conflict: ClientWriteConflictOptions{
				DeleteOnMissing: CLIENT_WRITE_REQUEST_DELETES_ON_MISSING_ERROR,
			},
		}
		_, err := fgaClient.Write(context.Background()).Body(body).Options(options).Execute()
		if err != nil {
	...
	t.Run("Client Write with both WritesOnDuplicate and DeleteOnMissing options", func(t *testing.T) {
		test := TestDefinition{
			Name:           "Write",
			JsonResponse:   `{}`,
			ResponseStatus: 200,
	...
	t.Run("Client Write conflict options precedence test", func(t *testing.T) {
		...
		httpmock.RegisterResponder(test.Method, fmt.Sprintf("%s/stores/%s/%s", fgaClient.GetConfig().ApiUrl, getStoreId(t, fgaClient), test.RequestPath),
			func(req *http.Request) (*http.Response, error) {
				...
				if requestBody.Writes == nil || requestBody.Writes.OnDuplicate == nil || *requestBody.Writes.OnDuplicate != "ignore" {
					t.Errorf("Expected client WritesOnDuplicate option to override defaults, got %v", requestBody.Writes.OnDuplicate)
				}

				if requestBody.Deletes == nil || requestBody.Deletes.OnMissing == nil || *requestBody.Deletes.OnMissing != "ignore" {
					t.Errorf("Expected client DeleteOnMissing option to override defaults, got %v", requestBody.Deletes.OnMissing)
				}
